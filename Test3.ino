...
float setAngle = 0.0;
float curAngle = 0.0;
bool usePotentiometer = false;  // Set to false to use keyboard control
...
void checkKeyboardControl();  // NEW - Non-intrusive keyboard check
...
void setup() {
  ...
  if (!usePotentiometer) {
    Serial.println("Keyboard mode: Press 'i' to increase, 'k' to decrease hinge angle");
  }
}

// ============================================================================
// MAIN LOOP - IDENTICAL to original with minimal addition
// ============================================================================
void loop() {
  ...
  if (now - lastPIDUpdate >= 1) {
    ...
    // else setAngle is controlled by keyboard (checkKeyboardControl below)
  ...
  // ========== EVENT-DRIVEN: CAMERA TRIGGER (UNCHANGED FROM ORIGINAL) ==========
  if (triggerReceived) {
  ...
  // ========== NEW: KEYBOARD CONTROL (minimal, non-blocking) ==========
  if (!usePotentiometer) {
...
// ============================================================================
// NEW: MINIMAL KEYBOARD CHECK - Only touches 'i' and 'k', leaves rest alone
// ============================================================================
void checkKeyboardControl() {
  if (Serial.available() > 0) {
    char c = Serial.peek();  // Look but don't remove
    
    // Only handle our specific commands
    if (c == 'i' || c == 'I') {
      Serial.read();  // Now remove it
    ...
    // Don't touch any other characters - leave them for camera
  }
}
