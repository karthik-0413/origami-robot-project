...
struct Motor {
  int in1, in2;
  Motor(int in1_, int in2_) : int1(in1_), in2(in2_) {}
};
...
const float HINGE_ANGLE_STEP = 5.0;    // Degrees to change per keystroke
const float HINGE_MIN_ANGLE = -180.0;  // Minimum hinge angle
const float HINGE_MAX_ANGLE = 180.0;   // Maximum hinge angle
...
uint8_t ackMsg = 1;
float setAngle = 0.0;  // Now controlled by keyboard instead of potentiometer
float curAngle = 0.0;
...
void increaseHingeAngle();
void decreaseHingeAngle();
...
void setup() {
  ...
  Serial.println("===========================================");
  Serial.println("Hinge Control Commands:");
  Serial.println("  'i' or 'I' = Increase angle by +5°");
  Serial.println("  'k' or 'K' = Decrease angle by -5°");
  Serial.println("===========================================");
  Serial.println("Just PRESS the key once (don't hold)");
...
void loop() {
  ...
  if (now - lastPIDUpdate >= 1) {
    ...
    // No longer reading potentiometer - setAngle is controlled by keyboard
  ...
  if (triggerReceived) {
    ...
    Serial.println("[CAMERA] Trigger received - capturing...");
    captureAndSend();  // This should handle WiFi/UDP communication
    
    esp_err_t res = esp_now_send(receiverMAC, &ackMsg, 1);
    if (res == ESP_OK) {
      Serial.println("[CAMERA] ACK sent successfully");
    } else {
      Serial.print("[CAMERA] ACK send failed, error: ");
      Serial.println(res);
    }
    delay(50);
  }
  
  // ========== HINGE COMMAND HANDLING (from ESP-NOW) ==========
  if (hingeCmdReceived) {
    hingeCmdReceived = false;
    handleHingeCommand();
  }
  
  // ========== POSITION COMMAND HANDLING (from ESP-NOW) ==========
  if (positionCmdReceived) {
    positionCmdReceived = false;
    handlePositionCommand();
  }
  
  // ========== SERIAL COMMAND HANDLING (at the END to avoid interference) ==========
  // Only check for hinge control commands - don't interfere with camera data
  handleSerialCommands();
}

// ============================================================================
// SERIAL COMMAND HANDLING - Only for hinge control 'i' and 'k'
// ============================================================================
void handleSerialCommands() {
  // Check if there's data available
  if (!Serial.available()) return;
  
  // Peek at the character without removing it
  char c = Serial.peek();
  
  // Only consume the character if it's a hinge command
  if (c == 'i' || c == 'I' || c == 'k' || c == 'K') {
  ...
  // Leave other characters in the buffer for camera module to handle
}

// ============================================================================
// INCREASE HINGE ANGLE
// ============================================================================
void increaseHingeAngle() {
  float oldAngle = setAngle;
  setAngle += HINGE_ANGLE_STEP;
  
  // Clamp to maximum
  if (setAngle > HINGE_MAX_ANGLE) {
    setAngle = HINGE_MAX_ANGLE;
  }
  
  Serial.println("===========================================");
  Serial.print("[HINGE] Setpoint: ");
  Serial.print(oldAngle);
  Serial.print("° → ");
  Serial.print(setAngle);
  Serial.println("°");
  Serial.print("[HINGE] Current angle: ");
...
void decreaseHingeAngle() {
  ...
  // Clamp to minimum
  if (setAngle < HINGE_MIN_ANGLE) {
    setAngle = HINGE_MIN_ANGLE;
  }
...
// ============================================================================
// ESP-NOW CALLBACK - Enhanced to handle multiple packet types
// ============================================================================
...
// ============================================================================
// Execute Motor Commands (Mecanum/Omnidirectional Drive)
// ============================================================================
void executeMotorCommands(float linearX, float linearY, float angularZ) {
