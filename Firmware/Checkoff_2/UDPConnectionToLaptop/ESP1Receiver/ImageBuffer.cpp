 ...
 * Description: Processes complete image and forwards via UDP to laptop.
 ****************************************************/
void finalizeImage(ImageBuffer &ib) {
    ...
    // Forward to laptop in UDP chunks (max 1400 bytes per packet)
    const size_t UDP_CHUNK = 1400;
    const size_t HEADER_SIZE = 9;  // START(4) + CAMERA_ID(1) + SIZE(4)
    ...
    Serial.printf("Camera %u - Forwarded %u bytes (%u chunks) - FPS: %.2f\n", 
                  ib.camera_id, totalSize, totalChunks, fps);
