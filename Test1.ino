...
unsigned long lastSerialCheck = 0;  // ⭐ NEW - Rate limit serial checking
...
void setup() {
  ...
  Serial.println("Hinge Control: 'i' = increase angle (+5°), 'k' = decrease angle (-5°)");
  Serial.println("Note: Just PRESS the key once (don't hold it down)");
  Serial.println("Current setpoint: 0.0°");
}
...
void loop() {
  ...
  // ========== SERIAL COMMAND HANDLING (rate limited to every 50ms) ==========
  if (now - lastSerialCheck >= 50) {
  ...
  // ========== EVENT-DRIVEN: CAMERA TRIGGER (HIGHEST PRIORITY) ==========
  if (triggerReceived) {
    triggerReceived = false;
    Serial.println("Camera trigger received - capturing image...");
    captureAndSend();
...
// ============================================================================
// ⭐ IMPROVED: SERIAL COMMAND HANDLING
// ============================================================================
void handleSerialCommands() {
  ...
  while (Serial.available() > 0) {
    ...
    switch (c) {
      ...
      default:
        // Only print for printable characters
        if (c >= 32 && c <= 126) {
...
void increaseHingeAngle() {
  ...
  Serial.print("Hinge setpoint increased to: ");
...
void decreaseHingeAngle() {
  ...
  Serial.print("Hinge setpoint decreased to: ");
  Serial.print(setAngle);
  Serial.print("° (Current: ");
  Serial.print(curAngle);
  Serial.println("°)");
